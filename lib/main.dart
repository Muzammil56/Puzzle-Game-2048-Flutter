...
import 'package:puzzle_game_2048/model.dart';
import 'package:puzzle_game_2048/tile-colors.dart';
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: Scaffold(
        backgroundColor: Color(0xff9FD9CB),
        body: BoardWidget(
        ),
      ),
    );
...
class _BoardWidgetState extends State<BoardWidget> {
  ...
  void initState(){
    ...
    _isMoving = false;
    gameOver = false;
    _board = Board(row: row ,column: column );
    newGame();
  }
  void newGame(){
    setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    for(int r = 0; r < row; ++r ) {
      for(int c = 0; c < column; ++c){
        _tileWidgets.add(
            TileWidget(tile: _board.getTile(r, c),
              state: this,)
        );
      }
    }
...
class AnimatedTileWidget extends AnimatedWidget {
  final Tile tile;
  final _BoardWidgetState state;
  AnimatedTileWidget({Key key, this.tile, this.state, Animation<double> animation})
      : super(
    key: key,
  ...
  Widget build(BuildContext context) {
    ...
    if(tile.value == 0){
      ...
      return TileBox(
        ...
        color: tileColors.containsKey(tile.value) ? tileColors[tile.value] :Colors.teal[50],
